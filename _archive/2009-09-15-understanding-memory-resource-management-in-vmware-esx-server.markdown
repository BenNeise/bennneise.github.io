...
- **Ballooning**. Is where VMware tools allows the hypervisor to see inside the guest operating system and reclaim unused memory. This typically occurs when ESX drops to less than 4% free memory (the _Soft_ threshold). It has more of an overhead than TPS, but is still preferable to the alternative.
